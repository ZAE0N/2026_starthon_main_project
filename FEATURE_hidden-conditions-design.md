...
### (1) 야간수당은 판정 항목이 아니다 — 안내 전용으로 결정 (2026-09-14)

기획의 대표 예시가 야간수당인데, `laws.json` 의 `checks` 8개에 **근로기준법 제56조가 없다.**
`scenario/README.md` 에도 적혀 있다. 시나리오3 계약서의 진짜 문제가 제56조 위반인데
앱은 휴게시간만 짚는다.

**야간수당은 "몰랐을 수도 있는 것" 안내로만 다룬다. 판정 항목으로 만들지 않는다.**

제56조를 9번째 항목으로 올리면 `CheckId`, `CHECK_ORDER`, `CHECK_LABELS`,
`server/schema.py`, `laws.json`, 결과 화면 문구가 같이 바뀐다. 제출(9/15)까지
그걸 하고 검증까지 마칠 시간이 없다. 항목 개수를 바꾸면 지금 4/4 로 맞는
계약서 4종을 다시 다 재야 한다.

그래서 지켜야 할 것이 생긴다.

- **안내 문구에서 판정처럼 쓰지 않는다.** "야간수당을 안 주고 있어요" (X)
  → "야간수당은 5명 이상인 곳에서만 적용돼요" (O)
- 결과 화면에서 배지를 붙이지 않는다. 위법소지처럼 보이면 사용자가 위반으로 믿는다
- **발표에서 야간수당 판정을 할 수 있다고 말하지 않는다.** 못 잡는다.
  "이 조건을 먼저 묻는다" 가 우리가 내세울 것이고, 판정한다는 말은 하지 않는다
- `PROGRESS.md` 의 "개선 방향" 에 제56조 항목 추가가 그대로 남는다.
  심사에서 물으면 범위에서 뺀 이유를 그 문서로 답한다
...
1. ~~야간수당을 안내 전용으로 둘지~~ → **안내 전용으로 결정 (2026-09-14).** 위 (1) 참고
